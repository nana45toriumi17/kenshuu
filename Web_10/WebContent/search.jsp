...
<table border="1">
<tr>
	<th>番号</th><th>タイトル</th><th>タスク内容</th><th>期限</th>
	<th>最終更新</th><th>ユーザID</th><th>状況</th><th>詳細画面</th><th>添付ファイル</th>
</tr>
...
<tr>
	...
	<td><fmt:formatDate value="${vo.limitdate}" pattern="yyyy-MMM-dd" /></td>
	<td><fmt:formatDate value="${vo.lastupdate}" pattern="yyyy-MMM-dd" /></td>
	<td><c:out value="${vo.userid}" /></td>
	<td><c:out value="${vo.label}" /></td>
	<td><a href="detail?id=<c:out value="${vo.id}" />">詳細画面へ</a></td>
	<td><c:choose><c:when test="${vo.filename !=null}">
